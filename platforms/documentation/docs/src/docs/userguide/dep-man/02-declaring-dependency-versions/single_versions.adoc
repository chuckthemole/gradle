...
The symbol `]` can be used instead of `(` for an exclusive lower bound and `[` instead of `)` for an exclusive upper bound. e.g. `]1.0, 2.0[`. Note: The range `],2.0[` is not appropriate, as it lacks a lower bound, and can be written as `(,2.0[`.
